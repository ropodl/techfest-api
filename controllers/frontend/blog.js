...
exports.blogs = async (req, res) => {
    const paginatedBlog = await paginate(BlogSchema, 1, 12, { status:"Published" }, { createdAt: '-1' });
    const blogs = await Promise.all(paginatedBlog.documents.map(async (blog) => {
        const { id, title, slug, featuredImage } = blog;
        return {
...
exports.blog = async (req, res) => {
    ...
    const {title,excerpt,content,featuredImage,categories,createdAt} = blog;
